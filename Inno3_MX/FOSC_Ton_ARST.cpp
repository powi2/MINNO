...
//				ddd_7->ddd_set_hi_level(gCVtgt_15pct_Higher);				// Will immediately change to this level upon execution
			...
			//Setup to have Secondary power up to VBPS minus
				...
				if (gTrim_VR_S == 1) 
					VO_dvi->set_voltage(VO_ch, 4.8, 		VOLT_5_RANGE);	// DVI_9_0
				////FB (1V)
...
if(g_TurretCB)
{
	...
	//lower Vout for internal vfb parts
	if (gTrim_VR_S == 1)	//internal vfb resistors
	...
	//if(LB_Rev >= 5)			ddd_7->ddd_set_hi_level(gCVth_10pct_Higher);
	if(LB_Rev >= 5)			ddd_7->ddd_set_hi_level(1.7);
	else					ddd_7->ddd_set_hi_level(gCVth_10pct_Higher);
	delay(1);
	//ddd_7->ddd_set_hi_level(gCVtgt_15pct_Higher);	//15% above CV(1.265V) to turn on 200mA bleeder at Vout (Make sure < 20% to avoid Latch off)
	ddd_7->ddd_run_pattern(Low_XXX_pat_start, Low_XXX_pat_stop);
	wait.delay_10_us(10);
...
////////////
	...
	//	wait.delay_10_us(5);
		...
		if(1)
		//if(gTrim_VR_S == 0)		//EXTERNAL FB device option ONLY
		{
			if(gTrim_VR_S > 0)		//INTERNAL FB device option ONLY
			{
				int Vout_vRange=0;

				if(gTrim_VR_S == 1)		Vout_vRange = VOLT_10_RANGE;
				if(gTrim_VR_S == 2)		Vout_vRange = VOLT_10_RANGE;
				if(gTrim_VR_S == 3)		Vout_vRange = VOLT_20_RANGE;
				VO_dvi->set_voltage(VO_ch, gCV_VO_S*1.01, 		Vout_vRange);	//Purposely set 1% above CVth_Vout for use of CVth_FB for internal FB
				VPIN_ovi->set_voltage(VPIN_ch, 5.0,   RANGE_5_V);
				wait.delay_10_us(20);  
				ddd_7->ddd_set_hi_level(1.7);		//stay away from 2% and 10% so the current bleeder won't turn on and cause Vout switching
				ddd_7->ddd_set_lo_level(0.9);		
				//delay(1);
				wait.delay_10_us(10);
			}
...
//	wait.delay_10_us(100);  	
	...
	if (gTrim_VR_S == 1) 	//internal fb resistors		//better levels to measure 10V off-time pulse
	{
		tmu_6->start_trigger_setup(5.0, POS_SLOPE, TMU_HIZ, TMU_IN_25V);//better levels to measure 10V off-time pulse
		tmu_6->stop_trigger_setup(4.0, NEG_SLOPE, TMU_HIZ, TMU_IN_25V);	//better levels to measure 10V off-time pulse
	}
	tmu_6->start_holdoff(0,FALSE);
	tmu_6->stop_holdoff(0,FALSE);
	...
	if (gTrim_AR_req && (OPCODE==4200 || OPCODE==4250))	// Per JD, this test is not working at HOT, we need to fix this at HOT
	{
