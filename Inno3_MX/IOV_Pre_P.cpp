...
void IOV_Pre_P(test_function& func)
{
	...
	float	CV_FB_Init_pt_S	=0.0;
			gCV_FB_Init_pt_S=0.0; //initialize this variable if 4200 or g_GRR 
...
//	search_CV_DDD(&CV_FB_Init_pt_S);
	search_CV_TM18(&CV_FB_Init_pt_S, Srch_CV);


	/*********************************************************************
	// DONE searching for CV_Init
	/*********************************************************************/
//delay(1);
	//pulse.do_pulse();
	...
	//Prevent OVI over range.
	if( fabs(CV_FB_Init_pt_S) < 2 )
		gCV_FB_Init_pt_S = CV_FB_Init_pt_S;
	else
		//gCV_FB_Init_pt_S = gCV_FB_TARGET;
		gCV_FB_Init_pt_S = 1.8;	//prevent over-range throut program
	gCVth_5pct_Lower	= gCV_FB_Init_pt_S * 0.95;	//Above highest AR 90% threshold
	gCVth_5pct_Higher	= gCV_FB_Init_pt_S * 1.05;	//
	gCVth_8pct_Higher	= gCV_FB_Init_pt_S * 1.08;	//
	gCVth_10pct_Higher	= gCV_FB_Init_pt_S * 1.10;
	gCVth_15pct_Higher = gCV_FB_Init_pt_S * 1.15;;
	...
	//datalog CV_FB_Init (skip if internal fb resistors used)
	if (gTrim_VR_S == 0)
		PiDatalog(func, A_CV_init_S,	CV_FB_Init_pt_S,		set_fail_bin,	POWER_UNIT);
