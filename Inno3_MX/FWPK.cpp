...
void FWPK(test_function& func)
{
	...
	if(!gTrim_DOPL_S && gSR_AF_post[gDOPL_AF_Addr_Start]==0)
	{
		...
		tmu_6->close_relay(TMU_CHAN_B_DUT2);	 // Connect TMU CHB to SR
		tmu_6->start_trigger_setup(0.9, POS_SLOPE, TMU_CHAN_B, TMU_IN_10V);	//SR	Channel_B_DUT2	to start
		tmu_6->stop_trigger_setup( 1.1, NEG_SLOPE, TMU_CHAN_B, TMU_IN_10V);	//SR	Channel_B_DUT2	to stop
		...
		REQ_ovi->set_voltage(REQ_ch, gREQ_V_RingDet, 		VOLT_5_RANGE);	// OVI_1_2
		wait.delay_10_us(4);
		...
		if (tDableOn_300kHz > 1) tDableOn_300kHz = 0.999;	// Prevent display error.
			...
			VO_dvi->set_voltage(VO_ch, 1, 		VOLT_5_RANGE);	// DVI_9_0  RLB
			...
			{
				VO_dvi->set_voltage(VO_ch, 1.15, 		VOLT_5_RANGE);	
			}
			else if(LB_Rev >= 5 && (OPCODE==4300||OPCODE== 4500))
			{
				VO_dvi->set_voltage(VO_ch, 1.05, 		VOLT_5_RANGE);	
			}
			else
			{
				if(OPCODE==4300||OPCODE== 4500)		VO_dvi->set_voltage(VO_ch, 1.05, 	VOLT_5_RANGE);
				else								VO_dvi->set_voltage(VO_ch, 1.15, 	VOLT_5_RANGE);
			}
