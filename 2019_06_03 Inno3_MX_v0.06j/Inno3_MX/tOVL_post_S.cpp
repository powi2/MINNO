...
void tOVL_post_S(test_function& func)
{
	...
	Func_name = "tOVL_post_S";

	gFuncNum++;
	if(gDisplay_FuncNum)
		PiDatalog(func, A_Func_Num_tOVL_post_S,		gFuncNum,		set_fail_bin,	POWER_UNIT);
...
//	int		vrng_b	=0.0;
	...
	SAMPLE_SIZE = GAGE_POST_256K;//GAGE_POST_4K;
	vrng_b = GAGE_PM_10_V;
	ChB_vrng = 5.6;
	//	Set up Buffer Clamp
...
//Setup to have Secondary power up to VBPS minus
	...
	{
		ddd_7->ddd_run_pattern(FB_tOVL_start,FB_tOVL_stop);
		wait.delay_10_us(5);	//HOT debug
		i++;
	}
	//wait.delay_10_us(20);	//good for avoiding dead zone after ~143us
	//wait.delay_10_us(50);	//HOT debug
	...
	if(g_TurretCB)
	{
		Gage_Find_tOVL_Turret(&tOVL_post_S);
	}
	else
	{
	...
	//Datalog
	PiDatalog(func, A_tOVL_post_S,		tOVL_post_S,			set_fail_bin,	POWER_MICRO);
