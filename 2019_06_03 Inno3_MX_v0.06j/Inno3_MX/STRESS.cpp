...
//	tmu_6->init();
 ...
 //   Set_I(res_BPS_dvi, 300 mA);
	...
	if(1)
	{
		...
		D_dvi->set_current(D_ch, 300e-3,		RANGE_300_MA);
		D_dvi->set_voltage(D_ch, 5.0, 			VOLT_50_RANGE);	// DVI_11_0, Need to be 5V for Minno Size x4
		wait.delay_10_us(50);
		...
		REQ_ovi->set_current(REQ_ch, 30e-3,	RANGE_30_MA);	
		//REQ_ovi->set_voltage(REQ_ch, gREQ_V_Idle,		RANGE_2_V);	// OVI_1_2	
		...
		if(LB_Rev >= 5)		ddd_7->ddd_set_hi_level(2);
		ddd_7->ddd_set_lo_level(1);
		wait.delay_10_us(100);
 ...
 ****************************************************************************/
	...
	if (PiGetAnyFailStatus() == false)	//Only trim Vshunt bit if Stress test passed
	{
		g_Stress_passed = 1;
	}
