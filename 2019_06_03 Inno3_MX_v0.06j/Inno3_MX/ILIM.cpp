...
void ILIM(test_function& func)
{
	...
	while (cnt++ < retest && ILIM_S < 100e-3)
	{
		if(DEBUG==1 && gDEBUG) printf("ILIM test# %d\n", cnt);
	...
	{
		cnt = 0;
		while (cnt++ < retest && ILIM_P1_S < 100e-3)
		{
			if(DEBUG==1 && gDEBUG) printf("ILIM_P1 test# %d\n", cnt);
			...
			else
			{
				...
				gIlim_Incr_max = gILIM_P1_TARGET_S;
			}
	 ...
	 ********************************************************************************/
			...
			Gage_Find_Ilim_MINNO(&ILIM_peak_rslt, &Dummy_didt, &Dummy_ton, &Rdson_peak_rslt, &Dummy_rdson_spec);
	...
	cnt = 0;
	while (cnt++ < retest && (ILIM_Jit18K_S < 71 || ILIM_Jit18K_S > 96))
	{
		if(DEBUG==1 && gDEBUG) printf("ILIM_Jit18K test# %d\n", cnt);
	...
	if(gTrim_SAM_P)
	{
		...
		cnt = 0;
		while (cnt++ < retest && ILIM_70pct_mA_P < 100e-3)
		{
			if(DEBUG==1 && gDEBUG) printf("ILIM_SSS test# %d\n", cnt);
	...
	{
		// Special limits for SC1885C, INN3468c-HY09
		...
		LSL = gILIM_TARGET_S * 0.94;
		USL = gILIM_TARGET_S * 1.06;
		func.dlog->set_test_no(A_ILIM_S + 1);
		func.dlog->set_min_limit_value(LSL);
		func.dlog->set_max_limit_value(USL);
		PiDatalog(func, A_ILIM_S,						ILIM_S,						set_fail_bin,	POWER_MILLI);

		LSL = (gILIM_TARGET_S / 4) - 0.05;
		USL = (gILIM_TARGET_S / 4) + 0.05;
		func.dlog->set_test_no(A_DIDT_S + 1);
		func.dlog->set_min_limit_value(LSL);
		func.dlog->set_max_limit_value(USL);
		PiDatalog(func, A_DIDT_S,						DIDT_S,						set_fail_bin,	POWER_MILLI);

		func.dlog->set_test_no(A_DIDT_P1_S + 1);
		func.dlog->set_min_limit_value(LSL);
		func.dlog->set_max_limit_value(USL);
		PiDatalog(func, A_DIDT_P1_S,					DIDT_P1_S,					set_fail_bin,	POWER_MILLI);

		LSL = (gILIM_TARGET_S * 0.4) - 0.250;
		USL = (gILIM_TARGET_S * 0.4) + 0.250;
		func.dlog->set_test_no(A_ILIM_200us_S + 1);
		func.dlog->set_min_limit_value(LSL);
		func.dlog->set_max_limit_value(USL);
		PiDatalog(func, A_ILIM_200us_S,					gILIM_200us_S,				set_fail_bin,	POWER_MILLI);

		LSL = gILIM_P1_TARGET_S * 0.93;
		USL = gILIM_P1_TARGET_S * 1.07;
		...
		LSL = (gILIM_P1_TARGET_S * 0.75) - 0.150;
		USL = (gILIM_P1_TARGET_S * 0.75) + 0.150;
	...
	else
	{
		...
		if (1)
		{
			...
			if (gILIM_TARGET_S == gILIM_P1_TARGET_S)
			{
				LSL = gILIM_P1_TARGET_S * 0.93;
				USL = gILIM_P1_TARGET_S * 1.07;
				func.dlog->set_test_no(A_ILIM_P1_S + 1);
		...
		if (gTrim_SAM_P)
		{
			if (strcmp("INN3468C-HR09", Dev_id) == 0 || strcmp("INN3468C-HY11", Dev_id) == 0 || strcmp("INN3468C-HZ12", Dev_id) == 0)
			{
				LSL = (gILIM_TARGET_S * 0.8) * 0.93;
				USL = (gILIM_TARGET_S * 0.8) * 1.07;
				func.dlog->set_test_no(A_ILIM_70pct_mA_P + 1);
				func.dlog->set_min_limit_value(LSL);
				func.dlog->set_max_limit_value(USL);
			}
